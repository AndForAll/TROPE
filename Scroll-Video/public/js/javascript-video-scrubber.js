...
var totalFrames = 502; // the number of images in the sequence of JPEG files (this could be calculated server-side by scanning the frames folder)
	...
	if(images.length > 0 && images[thisStep]) { // if the image exists in the array
		if(images[thisStep].complete) { // if the image is downloaded and ready
			if($('#video').attr('src') != images[thisStep].src) { // save overhead...?
				...
				$('#header').html("INSTRUCTION" + thisStep);
				console.log("STEP:" + thisStep);
			}
		}
	}
}
...
function changeInst(){
	...
	if(thisStep > 5 && thisStep < 25){
		$('#subheader').html("do shit with LED");
	}else
	if(thisStep > 25 && thisStep < 50){
		$('#subheader').html("do shit with WIRES");
	}else
	if(thisStep > 200 && thisStep < 400){
		$('#subheader').html("LOOK AT THIS COOL CIRCUIT");
	}else{
		$('#subheader').html(" ");
	}
